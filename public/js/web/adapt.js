...
/*
 * Минуты тоже приводим, а не только часы: у отрицательной минуты выходило
 * «23:-30» — на печатном листе это выглядело поломкой. Пока входы 0…1439,
 * разницы не видно, но лист печатают из чужих данных тоже.
 */
export const hhmm = (min) => {
  const м = ((min % 1440) + 1440) % 1440;
  return `${pad2(Math.floor(м / 60))}:${pad2(м % 60)}`;
};
