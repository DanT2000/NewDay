...
function отрисовать() {
   ...
   *
   * Полоса занимала место постоянно и пугала: «правки пока не уходят»
   * читалось как «правки потеряются». Теперь они не теряются — лежат в
   * очереди и уедут сами, — и сказать нужно другое и короче: сколько их
   * ещё в пути. Значка нет вовсе, когда всё уехало и связь на месте.
   */
  const вОчереди = data.ожидает();
  const беды = data.конфликты().length;
  const offline = (!noNet && !вОчереди && !беды) ? null : h('button.wsync',
    {
      type: 'button',
    ...
    h('span', {
      ...
      text: беды ? `${беды} не сохранилось`
        : (noNet ? `Нет связи${вОчереди ? ` · ${вОчереди}` : ''}` : String(вОчереди)),
    }));
