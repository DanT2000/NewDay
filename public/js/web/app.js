 ...
 */
/*
 * Номер перезагрузки. Рисует только последняя.
 *
 * Нажатий больше, чем ответов: человек листает дни, а каждый переход зовёт
 * `reload`. Раньше каждая перезагрузка дорисовывалась сама по себе, и та,
 * что вернулась позже, затирала экраном свои — уже устаревшие — данные.
 * Ответ на давнюю просьбу доезжает до локальной копии, но кисти не получает.
 */
let reloadGen = 0;
...
async function reload() {
  const gen = ++reloadGen;
  /*
   * День нужен каждому экрану, который день показывает.
   *
   * Раньше здесь стояло одно «Сейчас», а «Привычки» дописали отдельной
   * строкой ниже. Телефонную вкладку «Дела» — свой экран `tasks`, которого
   * на компьютере нет вовсе, — не дописал никто. И получалось так: человек
   * на «Делах» добавляет задачу, сервер её исправно сохраняет, `reload`
   * послушно ходит за объявлением и повторами, а день не перечитывает —
   * список на экране остаётся прежним. Задача «не добавилась». Удалённая
   * «не удалилась». Стоило листнуть полоску недели на другое число, как
   * экран продолжал показывать задачи прошлого дня, а новые уходили в дату
   * из шапки — отсюда и «добавил на 14, а появилось на 13».
   *
   * Перечисление одно на все экраны: добавится шестой — впишется сюда, а не
   * ещё одной строкой ниже, про которую забудут.
   */
  ...
  try {
    ...
    // «Делах» и «Привычках» их не видно — лишний запрос по мобильной сети
    if (state.screen === 'notes' || state.screen === 'today') jobs.push(data.loadNotes());
    ...
    // Пока ходили, человек ушёл дальше: рисовать наше уже нечего
    if (gen !== reloadGen) return;
    fill();
    render();
    /*
     * Нативные будильники ставим здесь, после каждого обновления данных.
     *
     * Веб-часть (эта самая) раньше этого не делала вовсе: расписание
     * показывалось, «дальше — подъём в 12:00» висело на экране, а в системный
     * планировщик Android не попадало ничего — и будильник просто не звонил.
     * `syncAlarms` сам берёт сегодня и завтра и пересчитывает; идемпотентен,
     * поэтому частые вызовы из `reload` только освежают уже стоящее. Только на
     * телефоне и не мешая рендеру: без плагина это тихо ничего не делает.
     */
    if (native.available()) native.syncAlarms(store.settings).catch(() => {});
...
function sideBar() {
  ...
  return h('aside.wside',
    ...
    nav,
    h('div.wside-foot',
      // висит над подвалом колонки и ничего в ней не сдвигает
      значокСвязи({ толькоБеда: true }),
      ai,
      themeBtn,
      /*
      ...
      h('div.wuser-body',
        ...
        /*
         * Подпись говорит правду о связи. Раньше здесь было зашито
         * «синхронизировано» — и оно же висело в метро без всякой связи.
         */
        h('div.wuser-note', { class: связь.вид, text: связь.текст })))));
}

/*
 * Состояние связи и очереди правок.
 *
 * Значок раньше стоял в потоке над содержимым и мигал на каждую галочку:
 * правка ложилась в очередь — значок «1» появлялся и сдвигал весь экран на
 * 35 пикселей вниз, через долю секунды правка уезжала — экран прыгал
 * обратно. Человек читал это как «страница обновляется» на каждое нажатие.
 *
 * Поэтому два правила. Первое: значок нигде не занимает места в
 * содержимом — на телефоне висит над нижней панелью, на компьютере над
 * карточкой в боковой колонке. Второе: обычная правка со связью не видна
 * вовсе. Показываем только то, о чём человеку нужно знать: связи нет,
 * очередь застряла дольше нескольких секунд или сервер правку отверг.
 */
const ЗАВИСЛА_МС = 3000;

function состояниеСвязи() {
  // место на устройстве — беда старше всех прочих: очередь встала совсем
  if (store.нетМеста) return { вид: 'bad', текст: 'На устройстве нет места' };
  const беды = data.конфликты().length;
  // «3 не сохранилось» верно при любом числе, в отличие от «3 правки не сохранились»
  if (беды) return { вид: 'bad', текст: `${беды} не сохранилось` };
  const вОчереди = data.ожидает();
  /*
   * Признак не только `navigator.onLine`: он врёт в обе стороны — «связь
   * есть» при точке без интернета и «нет» на некоторых оболочках Android.
   * Вторым признаком служит то, что день прочитан из местной копии.
   */
  if (navigator.onLine === false || store.offline) {
    return { вид: 'off', текст: вОчереди ? `Нет связи · ${вОчереди}` : 'Нет связи' };
  }
  const с = data.старейшаяПравка();
  if (вОчереди && с && Date.now() - с > ЗАВИСЛА_МС) {
    return { вид: 'wait', текст: `Сохраняю · ${вОчереди}` };
  }
  return { вид: 'ok', текст: 'синхронизировано' };
}

/**
 * Значок связи. Когда всё хорошо, его нет вовсе.
 *
 * `толькоБеда` — для компьютера: там о связи и так говорит подпись в
 * карточке, и дублировать её значком рядом незачем. Значок нужен лишь
 * тогда, когда нужно действие, — правка не сохранилась, и её надо открыть.
 */
function значокСвязи({ толькоБеда = false } = {}) {
  const с = состояниеСвязи();
  if (с.вид === 'ok' || (толькоБеда && с.вид !== 'bad')) return null;
  const значок = h('button.wsync', {
    type: 'button',
    class: с.вид,
    title: с.вид === 'bad' ? 'Есть правки, которые не сохранились'
      : с.вид === 'off' ? 'Связи нет: правки сохранены на устройстве и уедут сами'
        : 'Правки ещё едут на сервер',
    onclick: () => (с.вид === 'bad' ? set({ modal: 'sync' }) : data.отправитьОчередь()),
  });
  add(значок,
    ico(с.вид === 'bad' ? 'warning-circle-fill' : (с.вид === 'off' ? 'wifi-high' : 'arrows-clockwise'), '13px'),
    h('span', { text: с.текст }));
  return значок;
}
...
function statCards() {
  ...
  /*
   * Полоску третьей плитки считаем по неделе истории, которую сервер отдаёт
   * с каждой привычкой: число на плитке — серия, а полоска показывает, как
   * шла неделя.
   */
  // считаем только серии: у цели пропуск законен, и полоска спорила бы
  // с самим числом серии, падая от разрешённых пропусков
  const week = HABITS.filter(hb => hb.raw?.kind !== 'goal').flatMap(hb => hb.week ?? []);
   ...
   * Считает сервер: серия длиннее недели, а с привычками приезжает только
   * неделя истории. Процент за семь дней, стоявший здесь раньше, не
   * отличал «через день» от «три дня подряд, потом бросил».
   */
  const серия = Number(store.day?.habitsStreak ?? 0);

  const cards = [
    /*
     * Число сделанного, а не процент. Процент отвечал на вопрос, которого
     * человек не задавал: важно, сколько закрыто, а доля и так видна полоской.
     */
    {
      value: empty ? '—' : `${done}`,
      label: total ? 'завершено сегодня' : 'на этот день ничего не запланировано',
      p: percent,
    },
    {
      value: `${hd}/${act.length}`,
    ...
    {
      value: серия ? `${серия} ${adapt.plural(серия, 'день', 'дня', 'дней')}` : '—',
      label: серия ? 'серия привычек' : 'серии привычек пока нет',
      p: weekPct,
    },
  ];
...
function planTasks(dateKey) {
  ...
  add(wrap, ...rows.map(t => {
    const row = h('button.wptask', {
      ...
      class: t.done ? 'done' : '',
      title: t.text,
      onclick: () => busy(data.updateTask(dateKey, t.id, { done: !t.done })),
    });
...
function planScreen() {
  ...
  /*
   * Шапка расписания не уезжает вверх при прокрутке.
   *
   * День длинный: чтобы дойти до вечера, надо прокрутить сетку, и вместе с
   * ней уезжали и переключатель вида, и подписи дней. Человек листал колонку
   * и переставал понимать, какой это день недели, а чтобы переключиться на
   * соседний — возвращался наверх. Заголовок с переключателем и полоса дней
   * держатся у верхнего края, прокручивается только сама сетка.
   */
  return h('div', h('div.wplan-top', head, headRow), grid, tasksRow);
}
  ...
  add(grid, ...monthCells(y, m).map(c => {
    ...
    add(items, ...dayTasks.slice(0, MONTH_TASKS).map(t => {
      const line = h('button.wcell-task', {
        ...
        onclick: e => {
          e.stopPropagation();
          busy(data.updateTask(key, t.id, { done: !t.done }));
        },
      });
...
function alarmPanel() {
  ...
  if (advanced) {
    ...
    /*
     * Переключатели, а не выбор одного: включённых может быть несколько, и
     * каждый раз выпадает одна из них наугад. По умолчанию включены три —
     * человек включал «QR-код», думая, что выбрал его, а будильник давал то
     * пример, то значки. Говорим это прямо над подсказкой.
     */
    const pickNote = types.length > 1
      ? `Включено ${types.length}: каждый раз выпадает одна из них наугад. `
        + 'Чтобы всегда была одна и та же — оставьте включённой только её. '
      : 'Будильник всегда даёт эту задачу. ';
    add(panel, h('div.wpanel-label', { text: 'Задача пробуждения' }), taskList,
      h('div.wclock-cap', {
        text: pickNote
          + 'QR-код и шаги работают только на телефоне: камеры и датчика шагов в браузере нет. '
...
/** Когда предупредили про несохранённое: второе нажатие в ту же минуту выходит без них. */
let выходПредупреждён = 0;
...
async function logOut() {
  /*
   * Неотправленные правки уходят вместе с копией, поэтому сперва пробуем
   * их отдать. Если связи нет — говорим сколько их и просим нажать ещё
   * раз: молча стереть то, что человек уже видел сделанным, нельзя.
   */
  /*
   * Считаем и отвергнутые тоже: список «не сохранилось» — единственное
   * место, где эти правки ещё живы, и выход стирает его вместе с очередью.
   * Предупреждение действует минуту: человек мог прочитать его, уйти
   * работать и вернуться через час — молчать во второй раз нельзя.
   */
  const несохранённых = () => data.ожидает() + data.конфликты().length;
  if (несохранённых()) {
    await data.отправитьОчередь().catch(() => {});
    const осталось = несохранённых();
    if (осталось && Date.now() - выходПредупреждён > 60000) {
      выходПредупреждён = Date.now();
      note(`Не сохранилось на сервере: ${осталось}. Нажмите «Выйти» ещё раз, чтобы выйти без них.`);
      return null;
    }
  }
  data.forgetLocal();
...
function busy(job) {
  ...
  job.then(() => {
    ...
    /*
     * Рисуем сразу, не дожидаясь перезагрузки.
     *
     * Сервер уже ответил «сохранил» — значит шторке место за экраном. Раньше
     * `render` стоял только в конце `reload`, и всё то время, пока тот ходит
     * за днём, заметками, повторами и объявлением, на телефоне висела
     * застывшая кнопка «Сохраняю…». Человек нажимал ещё раз, потом ещё, и
     * решал, что сохранение не работает. Сама запись появится на своём месте
     * через мгновение — перезагрузка идёт следом.
     */
    render();
    return reload();
  }).catch(e => { state.busy = false; fail(e); });
...
const ALARM_PERMS = [
  ...
  },
  {
    ...
    why: 'без него экран будильника не поднимется, если телефоном в этот момент пользуются, и не вернётся сам, если его смахнуть «домой»',
    done: 'экран будильника поднимется поверх всего и вернётся, если его смахнуть',
    shown: () => true,
  },
  {
...
/*
 * Человеческое имя правки. Список почти всегда пуст и открывается один раз
 * в жизни — в тот момент, когда сервер отказал; поэтому важнее не
 * подробность, а понятность: что человек делал и в какой день.
 */
const ИМЯ_ПРАВКИ = {
...
const описаниеПравки = о =>
  `${ИМЯ_ПРАВКИ[о.вид] ?? 'Правка'} · ${adapt.shortDate(о.дата)}`;

const BODIES = {
  /*
   * Что не сохранилось. Открывается только со значка и только когда сервер
   * отказал по существу: строку удалили на другом устройстве, день не
   * прошёл проверку. Всё остальное очередь довозит сама и молча.
   */
  sync() {
    const список = data.конфликты();
    const тело = h('div.wpanel',
      h('div.wpanel-note', {
        text: список.length
          ? 'Эти правки сервер не принял. Можно попробовать ещё раз или забыть их — '
            + 'тогда на экране останется то, что лежит на сервере.'
          : 'Всё сохранено.',
      }),
      h('div.wdevs', ...список.map(о => h('div.wdev',
        ico('warning-circle-fill', '17px'),
        h('div.wdev-body',
          h('div.wdev-name', { text: описаниеПравки(о) }),
          h('div.wdev-seen', { text: о.ошибка ?? 'Не сохранилось' })),
        h('button.wbtn-line', {
          type: 'button', text: 'Повторить',
          onclick: () => { data.повторитьПравку(о.id); setIn({}); },
        }),
        h('button.wbtn-line', {
          type: 'button', text: 'Забыть',
          onclick: () => { data.забыть(о.id); reload(); setIn({}); },
        })))));
    return тело;
  },
  ...
  habit() {
    ...
    return h('div.wstack',
      ...
      h('div',
        ...
        /*
         * У цели дни — план, а не обязательство, и об этом надо сказать
         * прямо: иначе это снова выбор без последствий, как прежняя пара
         * «выполнять/бросаю».
         */
        state.habitKind === 'goal' && state.habitPlan === 'days'
          ? h('div.wclock-cap', { style: { marginTop: '9px' },
            text: 'дни — это план: отметить можно и в другой день, всё пойдёт в счёт' })
          : null),
      h('div',
        h('div.wfield-label', {
...
/*
 * Как часто повторяется привычка: маска дней недели, понедельник — младший
 * бит. Так её считает сервер (`weekdayInMask`: бит = номер дня по ISO минус
 * один) и так же её собирает редактор привычки.
 *
 * Здесь стояли 62 и 65 — маски по другому счёту, где младший бит
 * воскресенье. Привычка «по будням» из шаблона получалась со вторника по
 * субботу, а «по выходным» — понедельник и воскресенье. Ошибка тихая:
 * в списке привычка выглядит обычной, и только в понедельник она молча
 * не спрашивается, а в субботу спрашивается зря.
 */
const HABIT_MASK = { daily: 127, weekdays: 31, weekend: 96 };
...
function отрисовать() {
  ...
  if (phone) {
    // значок связи висит над нижней панелью, а не в потоке содержимого
    const нав = phoneNav();
    const значок = значокСвязи();
    if (значок) add(нав, значок);
    replace(root,
      h('div.wpbody.wscroll', announce, notice, screens[state.screen]()),
      нав,
      modal(sameModal));
  } else {
    replace(root,
      sideBar(),
      h('div.wmain', topBar(),
        h('div.wbody.wscroll', announce, notice, screens[state.screen]())),
      modal(sameModal));
  }
 ...
 */
/*
 * Со связью сперва отдаём своё, потом забираем чужое: иначе перечитывание
 * принесёт день без наших правок, и они мигнут на экране.
 */
addEventListener('online', () => {
  store.offline = false;
  render();
  data.отправитьОчередь().finally(() => reload());
});
 ...
 */
/*
 * Застрявшую очередь показываем не сразу, а через ЗАВИСЛА_МС: обычная правка
 * уезжает быстрее, и мигать ради неё значком незачем. Отсюда таймер — без
 * него значок «Сохраняю» ждал бы случайной перерисовки.
 */
...
document.addEventListener('visibilitychange', () => {
  ...
  /*
   * Очередь пробуем отдать при каждом возврате, а не только когда связи не
   * было. На телефоне это главный повод: человек отметил дело в метро,
   * свернул приложение, вышел наверх и открыл снова — и ждёт, что всё уже
   * уехало. Сама по себе очередь досылает с нарастающей паузой, до минуты,
   * и минуту эту он бы просидел, глядя на счётчик.
   */
  if (data.ожидает()) data.отправитьОчередь();
