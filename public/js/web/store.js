...
export function запуститьОчередь() {
  ...
  очередь.подписаться(весть => {
    ...
    /*
     * Очередь не дозвонилась — значит связи нет, что бы ни говорил браузер.
     * Признак нужен значку: он показывает «Нет связи» вместо «Сохраняю».
     */
    if (весть.вид === 'нетСвязи') { store.offline = true; сообщить(); return; }
    /*
...
/**
 * Поля самого человека: тема, начало недели, вид расписания.
 *
 * Тоже через очередь — и по той же причине, что и переключатели: без связи
 * выбор должен оставаться выбором, а не откатываться красной полосой.
 */
