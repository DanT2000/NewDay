    ...
    async send(subscription, payload) {
      ...
      try {
        await webpush.sendNotification(
          ...
          /*
           * Таймаут обязателен. Без него зависший push-сервис (принял
           * соединение и молчит) держал отправку до таймаута операционной
           * системы — минутами. А отправка идёт в одном цикле по всем
           * подписям всех людей: один мёртвый адрес останавливал рассылку
           * целиком, следующий тик планировщика брал те же записи и слал
           * их повторно.
           */
          { TTL: 600, urgency: 'high', timeout: 10000 },
        );
        return { ok: true };
      } catch (e) {
