...
module.exports = function adminPanelRouter({ db, config, ai, access, push, cleanup }) {
  ...
  // ── Объявление для всех вошедших ───────────────────────────

  /*
   * Полоса с текстом поверх приложения: владелец пишет обычными словами со
   * смайликами, включает — и её видят все. Текст отдаётся панели целиком
   * даже при погашенном тумблере: иначе форма при каждом открытии показывала
   * бы пустое поле, и включить заготовленное объявление было бы нечем.
   */
  router.get('/announce', wrap((_req, res) => res.json(panel.announce())));

  router.put('/announce', wrap((req, res) => {
    // Пустой текст — законное значение, а не недосмотр: это и есть способ
    // стереть объявление. Никакой чистки и экранирования: смайлики должны
    // доехать теми же символами, что набрал админ, — полоса печатает текст,
    // а не разметку.
    const text = v.str(req.body?.text, { max: 2000, field: 'текст объявления' });
    // v.bool, а не Boolean: строка 'false' — это ложь, которую Boolean читает
    // как правду, а тумблер приходит и формой, и JSON
    res.json(panel.setAnnounce({ text, on: v.bool(req.body?.on) }));
  }));
