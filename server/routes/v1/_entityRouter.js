...
function entityRouter({ db, repoFor, sanitize, extra }) {
  ...
  router.post('/', wrap((req, res) => {
     ...
     */
    /*
     * Ключ считаем вместе с путём.
     *
     * Сам по себе он был общим на весь аккаунт, и один и тот же ключ на разных
     * путях означал «это тот же запрос»: интеграция со счётчиком вида «op-5»
     * создавала задачу, а следующий запрос — приём пищи с тем же ключом —
     * получал в ответ ту самую задачу и код 201. Приём пищи не создавался
     * вовсе, а клиент подставлял себе чужой номер строки.
     */
    const ключ = ключи.ключИз(req.get('idempotency-key'));
    const полный = ключ && `${req.baseUrl}|${ключ}`;
    const был = полный && ключи.повтор(req.user.id, полный);
    ...
    if (полный) ключи.запомнить(req.user.id, полный, 201, row);
