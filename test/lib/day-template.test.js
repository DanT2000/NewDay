...
/*
 * День из одного расписания. Так 15 сентября выглядел вечерний план на
 * завтра: строка «День:» и восемнадцать строк под «Расписание:». Требование
 * двух разделов отправляло его к модели, и человек получал «Не удалось
 * соединиться с моделью» вместо мгновенного разбора.
 */
const ОДНО_РАСПИСАНИЕ = `День: 2026-09-16
...
22:00–22:15 — NewDay: план на следующий день [будильник]
23:59 — Сон [напоминание]`;

test('день из одного расписания — тоже шаблон', () => {
  assert.equal(looksLikeTemplate(ОДНО_РАСПИСАНИЕ), true);
  // без строки «День:» узнаётся по строкам со временем — их три и больше
  assert.equal(looksLikeTemplate(ОДНО_РАСПИСАНИЕ.replace(/^День:.*\n/, '')), true);
  // «День:» без раздела — ещё не шаблон, как и раздел с парой строк
  assert.equal(looksLikeTemplate('День: завтра\nсозвон в девять'), false);
  assert.equal(looksLikeTemplate('Расписание:\n09:00–10:00 — зарядка\n10:00–11:00 — завтрак'), false);

  const { items, date } = parseDayTemplate(ОДНО_РАСПИСАНИЕ, { date: '2026-09-15' });
  assert.equal(date, '2026-09-16');
  assert.equal(items.length, 6);
  const молодёжка = items.find(i => i.title === 'Молодёжка');
  assert.deepEqual([молодёжка.kind, молодёжка.start, молодёжка.end], ['schedule', '19:00', '22:00']);
  assert.equal(items.find(i => i.title.startsWith('NewDay')).alarm, 'alarm');
  assert.equal(items.find(i => i.title === 'Сон').kind, 'reminder');
});
