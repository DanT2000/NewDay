...
test('напоминания приводятся так же, как их приведёт сервер', async () => {
  const { наложить } = await import('../../public/js/web/apply.js');
  /*
   * Сервер дедуплицирует сроки, сортирует по убыванию и оставляет не больше
   * шести. Местная строка обязана выглядеть так же: иначе после отправки
   * метки перескакивают в другой порядок — то самое «дёрганье», от которого
   * очередь и должна избавлять.
   */
  const стало = наложить(день(), оп('строка.создать', {
    цель: 'tmp-3',
    данные: { раздел: 'schedule', поля: { title: 'Встреча', remindBefore: [0, 15, 60, 15] } },
  }));
  assert.strictEqual(стало.schedule.at(-1).remind_before_json, '[60,15,0]');
  assert.strictEqual(стало.schedule.at(-1).remind_before_min, 60, 'первый срок продублирован, как у сервера');
});
...
test('список заметок видит неуехавшую правку заметки дня', async () => {
  const { наложитьНаЗаметки } = await import('../../public/js/web/apply.js');
  const список = [
    { id: '2026-09-20', date: '2026-09-20', title: 'завтра', text: '' },
    { id: '2026-09-18', date: '2026-09-18', title: 'позавчера', text: '' },
    { id: 7, title: 'свободная', text: 'без даты' },
  ];
  const поля = (дата, notes) => ({ id: 'op', вид: 'день.поля', дата, данные: { поля: { notes } } });

  const правка = наложитьНаЗаметки(список, [поля('2026-09-18', 'Сервис ноутбука\nСпросить про сроки')]);
  assert.deepStrictEqual(
    [правка[1].title, правка[1].text], ['Сервис ноутбука', 'Спросить про сроки'],
    'заголовок — первая строка, как у сервера');

  const новая = наложитьНаЗаметки(список, [поля('2026-09-19', 'новая')]);
  assert.deepStrictEqual(новая.filter(x => x.date).map(x => x.date),
    ['2026-09-20', '2026-09-19', '2026-09-18'], 'новая встала на своё место по дате');

  const стёрта = наложитьНаЗаметки(список, [поля('2026-09-20', '   ')]);
  assert.ok(!стёрта.some(x => x.date === '2026-09-20'), 'пустая заметка из списка уходит — как у сервера');

  const чужое = наложитьНаЗаметки(список, [поля('2026-09-18', undefined)]);
  assert.deepStrictEqual(чужое, список, 'правка веса или плана питания список заметок не трогает');
  assert.strictEqual(список[1].title, 'позавчера', 'исходный список не тронут');
});
