...
test('неуехавшая правка не задваивается при перечитывании дня', async () => {
  const { data } = await стенд();
  const с = сеть();
  /*
   * Местная копия обязана хранить ответ сервера, а не то, что видно на
   * экране. Иначе очередь накладывается на собственный результат: задача,
   * добавленная в метро, показывалась дважды после первой же перерисовки —
   * а человек, приняв это за сбой, удалял «лишнюю» и терял обе.
   */
  с.обрыв();
  data.createTask('2026-09-19', { text: 'хлеб', bucket: 'home' });
  for (let i = 0; i < 3; i++) await data.loadDay('2026-09-19').catch(() => {});
  assert.strictEqual(data.store.day.tasks.home.length, 1, 'строка одна, сколько ни перечитывай');
});
