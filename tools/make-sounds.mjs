/**
 * ЗАПАСНОЙ ПУТЬ. В сборку идут не эти звуки.
 *
 * В public/sounds/ лежат настоящие записи из открытых источников (AOSP под
 * Apache-2.0, Wikimedia Commons и freesound под CC0 и public domain) — откуда
 * что взято, написано в public/sounds/README.md. Синтез людям не понравился:
 * узнаваемо electronic, «пищит, а не звенит».
 *
 * Скрипт оставлен на случай, когда готовых записей нет под рукой: нужен звук
 * в офлайне, не проходит лицензия, не работает сеть. Учтите, что он ПЕРЕЗАПИШЕТ
 * public/sounds/manifest.json своим набором из восьми WAV — вместе с полем
 * license и ссылками на .ogg. Перед запуском стоит сохранить манифест, а после
 * вернуть, иначе набор из README и то, что лежит на диске, разъедутся.
 *
 ...
 *
 * Почему здесь синтез: у синтеза воспроизводимость. Любой звук можно
 * поправить правкой числа и перегенерировать, и в репозитории лежит не
 * бинарный артефакт неизвестного происхождения, а его исходник. Минус —
 * ровно тот, из-за которого набор и заменили: синтез слышно.
 *
