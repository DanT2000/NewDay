...
// не «августа» намертво: проба должна жить в любой месяц — проверяем форму даты
const ДАТА_ДНЯ = /^\d{1,2} (января|февраля|марта|апреля|мая|июня|июля|августа|сентября|октября|ноября|декабря) \d{4}, (понедельник|вторник|среда|четверг|пятница|суббота|воскресенье)$/;
проба('на листе стоит дата дня', ДАТА_ДНЯ.test(String(sheet?.head)), sheet?.head);
...
await wait(600);
/*
 * Разделов шесть, и «напоминаний» среди них нет нарочно: напоминание — это
 * строка расписания без конца, и живёт оно в расписании. Отдельный список
 * заставлял бы помнить, куда записана мысль. Тренировка, наоборот, свой
 * раздел: у упражнения три числа, галочкой с текстом его не записать.
 */
проба('на «Сейчас» шесть разделов и ни одного лишнего',
  (await js(`[...document.querySelectorAll('.wcap')].map(e => e.textContent).join(',')`))
    === 'расписание,задачи,питание,тренировка,привычки сегодня,заметки дня',
  await js(`[...document.querySelectorAll('.wcap')].map(e => e.textContent).join(',')`));
/*
...
/*
 * Задачи — переключатели, а не выбор одной, и по умолчанию их три. Человек
 * «выбрал» одну и удивлялся, что будильник даёт другие: теперь под списком
 * прямо сказано, что из включённых выпадает любая.
 */
const подписьЗадач = `[...document.querySelectorAll('.wclock-cap')]
  .find(e => e.previousElementSibling?.classList.contains('wstack-tight'))?.textContent ?? ''`;
проба('при нескольких задачах сказано, что выпадает любая',
  /Включено 3: .*наугад/.test(await js(подписьЗадач)), (await js(подписьЗадач)).slice(0, 70));
const переключитьЗадачу = имя => js(`[...document.querySelectorAll('.wrow-sw')]
  .find(r => r.querySelector('.wrow-sw-title')?.textContent === '${имя}').click()`);
await переключитьЗадачу('Код'); await wait(700);
await переключитьЗадачу('Значки'); await wait(700);
проба('при одной задаче сказано, что она будет всегда',
  (await js(подписьЗадач)).startsWith('Будильник всегда даёт эту задачу'), (await js(подписьЗадач)).slice(0, 50));
await переключитьЗадачу('Код'); await wait(700);
await переключитьЗадачу('Значки'); await wait(700);
/*
...
// сравниваем «до» и «после», а не абсолютное число: стенд может быть не пуст
проба('задачи не тронуты', задачВ(послеШ) === задачДо, `${задачДо} → ${задачВ(послеШ)}`);
...
/*
 * Полосу с объявлением ставит владелец из панели, а видят её все вошедшие.
 * Проверяем ту самую последовательность, ради которой объявление сделано через
 * номер редакции: человек прочитал и закрыл — владелец переписал — человек
 * увидел снова.
 *
 * Видимость меряем настоящую: `offsetParent` («участвует в раскладке») и
 * размеры прямоугольника («занимает место»). На наличии узла и на свойстве
 * `hidden` здесь уже обманывались: в разметке лежит, а человек не видит —
 * нулевая высота, скрытый родитель, `display: none` из чужого правила. Ни
 * querySelector, ни `hidden` об этом не расскажут, а offsetParent расскажет.
 *
 * Ставим объявление запросом к панели прямо со страницы: origin тот же, cookie
 * браузер сохраняет сам, а пользовательский вход при этом не теряется — панель
 * лишь ставит свою отметку в ту же сессию. Пароль на стенде по умолчанию: база
 * стенда одноразовая, хеша пароля в ней нет.
 */
const ОБЪЯВЛЕНИЕ_1 = 'Стенд переезжает в субботу 🚀';
const ОБЪЯВЛЕНИЕ_2 = 'Переезд перенесли на воскресенье';

const полосаОбъявления = () => js(`(() => {
  const n = document.querySelector('.wannounce');
  if (!n) return { видно: false, почему: 'узла нет', текст: '', детей: -1 };
  const r = n.getBoundingClientRect();
  const t = n.querySelector('.wannounce-text');
  return {
    видно: Boolean(n.offsetParent) && r.width > 1 && r.height > 1,
    почему: 'в раскладке: ' + Boolean(n.offsetParent)
      + ', ' + Math.round(r.width) + '×' + Math.round(r.height),
    текст: t?.textContent ?? '',
    // сколько узлов родилось внутри текста: у честного textContent — ни одного
    детей: t ? t.children.length : -1,
  };
})()`);
const поставитьОбъявление = (text, on) => js(`fetch('/api/admin/announce', {
  method: 'PUT', headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({ text: ${JSON.stringify(text)}, on: ${on} }),
}).then(r => r.json())`, true);
/*
 * «Полоса доехала до уже открытой страницы» проверяем событием `online`, а не
 * `visibilitychange`, — и вот почему.
 *
 * Приложение нарочно молчит, пока страницу не смотрят: и обработчик возврата,
 * и пятиминутный таймер сперва спрашивают `visibilityState`. А страница этого
 * прогона считает себя скрытой — безголовое окно без фокуса, — поэтому
 * `visibilitychange` здесь честно не делает ничего, и проба мерила бы не
 * объявление, а видимость окна (на этом уже потеряли полчаса: полоса не
 * возвращалась, хотя и сервер, и метка, и сам `api.announce()` были в порядке).
 * `online` же зовёт `reload`, а тот тянет объявление первым заданием — тот
 * самый путь, которым полоса приезжает к человеку с вернувшейся связью.
 */
const связьВернулась = async () => {
  await js(`dispatchEvent(new Event('online'))`);
  await wait(1400);
};
const открытьПриложение = async () => {
  await rpc(ws, 'Page.navigate', { url: `${BASE}/web.html` });
  await waitFor('Boolean(document.querySelector(".wside"))');
  await wait(1200);
};

const вошлиВПанель = await js(`fetch('/api/admin/login', { method: 'POST',
  headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ password: 'newday' }) })
  .then(r => r.status)`, true);
проба('панель пустила поставить объявление', вошлиВПанель === 200, `вход: ${вошлиВПанель}`);

const первое = await поставитьОбъявление(ОБЪЯВЛЕНИЕ_1, true);
await открытьПриложение();
await waitFor(`Boolean(document.querySelector('.wannounce'))`, 40);
const видимая = await полосаОбъявления();
проба('включённое объявление видно на экране', видимая.видно, видимая.почему);
проба('в полосе ровно тот текст, что набрал владелец',
  видимая.текст === ОБЪЯВЛЕНИЕ_1, JSON.stringify(видимая.текст));

await js(`document.querySelector('.wannounce-x')?.click()`);
await wait(600);
const послеКрестика = await полосаОбъявления();
проба('крестик убирает полосу', !послеКрестика.видно, послеКрестика.почему);
const метка = await js(`localStorage.getItem('newday.announce.seen')`);
проба('закрытие помечено номером редакции', метка === String(первое.rev),
  `метка: ${метка}, редакция: ${первое.rev}`);

/*
 * Перезагрузка — главная проверка метки: без неё «закрыл» жило бы только в
 * памяти страницы и объявление встречало бы человека при каждом входе.
 * Ждать даём долго нарочно: `waitFor` вернёт false, лишь просидев все попытки,
 * и проба «не вернулась» обязана дать полосе все шансы появиться, иначе она
 * измерит не поведение, а скорость машины.
 */
await открытьПриложение();
проба('после перезагрузки закрытая полоса не возвращается',
  !(await waitFor(`Boolean(document.querySelector('.wannounce'))`, 15)),
  (await полосаОбъявления()).почему);

/*
 * Новый текст сильнее «закрыто»: иначе владелец разговаривал бы со стеной со
 * всеми, кто однажды нажал крестик. Перезагрузку здесь не делаем — полоса
 * обязана доехать до уже открытой страницы.
 */
const второе = await поставитьОбъявление(ОБЪЯВЛЕНИЕ_2, true);
await связьВернулась();
const обновлённая = await полосаОбъявления();
проба('владелец переписал — полоса вернулась, хотя прежнюю закрывали',
  обновлённая.видно, `редакция ${первое.rev} → ${второе.rev}; ${обновлённая.почему}`);
проба('и в полосе новый текст, а не прежний',
  обновлённая.текст === ОБЪЯВЛЕНИЕ_2, JSON.stringify(обновлённая.текст));

await поставитьОбъявление(ОБЪЯВЛЕНИЕ_2, false);
await связьВернулась();
const послеВыключения = await полосаОбъявления();
проба('владелец выключил — полоса ушла', !послеВыключения.видно, послеВыключения.почему);

/*
 * Текст объявления — чужой ввод, и в полосу он попадает через `textContent`.
 * Проверяем это делом: угловые скобки обязаны остаться словами. Смотрим не на
 * текст, а на детей узла — разметка, которую браузер разобрал, видна как узлы,
 * а не как строка, и сравнение текстов её бы не поймало.
 */
const сРазметкой = 'Внимание: <b>жирным</b> не будет';
await поставитьОбъявление(сРазметкой, true);
await связьВернулась();
const чужаяРазметка = await полосаОбъявления();
проба('разметка в объявлении остаётся словами, а не превращается в html',
  чужаяРазметка.видно && чужаяРазметка.детей === 0 && чужаяРазметка.текст === сРазметкой,
  `детей внутри текста: ${чужаяРазметка.детей}; ${JSON.stringify(чужаяРазметка.текст)}`);

/*
 * Уборка. Объявление лежит в базе стенда, а не в профиле браузера: оставь его
 * висеть — и оно сдвинет раскладку под остальными пробами следующего прогона,
 * а красными они станут в местах, где ничего не сломано.
 */
await поставитьОбъявление('', false);
await js(`fetch('/api/admin/logout', { method: 'POST' }).then(r => r.status)`, true);
await открытьПриложение();
const наСервере = await js(`fetch('/api/v1/announce').then(r => r.json())
  .then(a => a.on + '/' + a.text)`, true);
